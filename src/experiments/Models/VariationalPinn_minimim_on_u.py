# -*- coding: utf-8 -*-
"""
Created on Tue Mar  8 10:33:34 2022
...
class VariationalOperator(Operator):
    ...
    def call_method(self, u: Callable, domain: List[tf.Tensor]):
        ...
        return 0.5 * ((D(derive_respect_to=[0])(u, domain)*tf.exp(-self.potential.eval(domain) / 2 / self.epsilon)-1/ 2 / self.epsilon*u(domain)*tf.exp(-self.potential.eval(domain) / 2 / self.epsilon)*self.potential.grad_eval(domain)) ** 2 \
                      + (u(domain)*tf.exp(-self.potential.eval(domain) / 2 / self.epsilon)) ** 2 * (
                              self.potential.lap_eval(domain) / 2 / self.epsilon +
                              self.potential.grad_eval(domain) ** 2 / 4 / (self.epsilon ** 2))
                      ) \
               - u(domain)*tf.exp(-self.potential.eval(domain) / 2 / self.epsilon) / self.epsilon * tf.exp(-self.potential.eval(domain) / 2 / self.epsilon)
...
class VariationalBoundaryOperator(Operator):
    ...
    def call_method(self, u: Callable, domain: List[tf.Tensor]):
        ...
        return (self.k / self.alpha + 1 / 2 / self.epsilon * self.potential.grad_eval(domain) * self.n(domain)) * (u(
            domain)*tf.exp(-self.potential.eval(domain) / 2 / self.epsilon))** 2 - 2 * \
               tf.exp(-self.potential.eval(domain) / 2 / self.epsilon) / self.alpha * self.g(domain) * u(domain)*tf.exp(-self.potential.eval(domain) / 2 / self.epsilon)


class VariationalPinnMinOnU(BaseExperiment):
    def __init__(self, epsilon, potential: Potencial, g: Callable, alpha: float, k: float, n: Callable,
    ...
    def __str__(self):
        return super(VariationalPinnMinOnU, self).__str__() + ("_ExpTerm" if self.exp_integration else "")
...
if __name__ == "__main__":
    experiment_path = Path.joinpath(results_path, 'VariationalPinnMinOnU')
    experiment_path.mkdir(parents=True, exist_ok=True)

    ve = VariationalPinnMinOnU(epsilon=EPSILON, potential=potential, g=g, n=n, k=k,
